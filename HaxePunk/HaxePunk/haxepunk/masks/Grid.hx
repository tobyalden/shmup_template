...
class Grid extends Hitbox
{
	 ...
	 * @return	tile value.
	 */
	public function getTile(column:Int = 0, row:Int = 0, defaultTo:Bool = false):Bool
	{
		if (usePositions)
		{
			column = Std.int(column / _tile.width);
		...
		return getTileXY(column, row, defaultTo);
	}
	 ...
	 * @return	tile value.
	*/
	function getTileXY(x:Int = 0, y:Int = 0, defaultTo:Bool = false):Bool
	{
		if (!checkTile(x, y)) return defaultTo;
		return data[y][x];
	}
